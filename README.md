...
My host OS being Linux Mint 22.3 MATE, I use the Ubuntu 24.04 version of VirtualBox.

At the time of writing, the related version of VirtualBox is 7.2.10.

Add the user to the *vboxusers* group, in order to let VirtualBox enumerate USB ports:
```
$ sudo adduser developer vboxusers
```

For macOS and Windows, check the documentation.

# Creation of the VM

[Download Linux Mint 22.3, MATE edition](https://linuxmint.com/download.php) ISO image.
...
* **Virtual machine name and operating system**:
  * **VM Name**: `LinuxMint` (or anything you prefer)
  * **ISO Image**: the Linux Mint ISO file
  * **Proceed with Unattended Installation**: uncheck
* **Specify virtual hardware**:
    ...
    * **Number of CPUs**: depending on your host machine. I set it to **2**
* **Specify virtual hard disk**:
    * Select **Create a Virtual Hard Disk**. I set size to 60 GB

Click the **Finish** button. Then, start the VM.

When GNU GRUB menu is displayed, click the Enter key.

Wait for Linux Mint desktop to be displayed.

If your keyboard layout is not QWERTY, click on the main-menu icon ![icon](images/linuxMintMenuIcon.png) (in the lower left-hand corner). Click the **Control Center** icon. Then click the **Keyboard** icon.

Select the **Layouts** tab. Add the layout for your keyboard, and remove the existing **English (US)** layout. Close the keyboard window, close the control center window.
...
In the VirtualBox menu, select **Devices > Insert Guest Additions CD image...**. A window asks you if you would like to run medium software. Click the **OK** button and then the **Run** button.
...
* Close the Software Manager window at the end of the installation
...
* **General**: tick **Use custom default terminal size** and set **Default size** to 120
* Close the window
...
* Tick **Auto-mount** and **Make Machine-permanent**
* Click the **OK** button

For the file manager, I prefer to get a list of files instead of icons. So:
* **Edit > Preferences...**
* In the **Views** tab, select **List View** for **View new folders using::**
* Close the window
